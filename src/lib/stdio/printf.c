...
{
        /*
         * No need to check (stream == NULL || fmt == NULL) since we dont rely
         * on these variable being non zero in THIS function. vfprintf may do
         * so, but that why vfprintf has its own check.
         */
...
int vfprintf(struct vfile* stream, char* fmt, va_list list)
{
        ...
        for (; *fmt != '\0'; fmt++, num++) {
                ...
                if (*fmt == '%') {
                        ...
                        int inc = 0;
                        /*
                         * Print the part of the format in between the
                         * last % and this one.
                         */
...
{
        /* No need to check (str == NULL || fmt == NULL) since we dont rely on
         * these variable being non zero in THIS function. vfprintf may do so,
         * but that why vfprintf has its own check.
         */
