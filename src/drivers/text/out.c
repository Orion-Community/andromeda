...
{
        int len = formatInt(buffer, index, base, (base == 10) ? TRUE : FALSE,
        FALSE); // only signed if base = 10
        buffer[len] = '\0';
        return buffer;
}
                ...
                boolean scientific)
{
        ...
        int precision = 0, // persents number after e (or E) when printing scientific.
                        count = formatInt(buffer, (int) num, base, TRUE,
                                        capital); // print part before dot.
        ...
        if (decimals == 0) // check for xx.0
                ...
                if (scientific) {
                        *((char*) buffer++) = (capital) ? 'E' : 'e'; //print exx or Exx
                        ...
                        count += formatInt(buffer, precision, base, TRUE,
                                        capital); //print precision
                }
