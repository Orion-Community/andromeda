...
{
        while((ctrl->status & 0x2) != 0) ol_ps2_update_status(ctrl);
...
{
        while((dev->status & 0x1) == 0) ol_ps2_update_status(dev);
        return inb(dev->read_port);
}
...
ol_ps2_write(ol_ps2_dev_t dev, uint8_t val, bool ack)
{
        ...
        {
                ol_ps2_update_status(dev);
                if((dev->status & 0x2) == 0) goto write;
        }
        return -1;
        ...
        for(i = 0; i < 10; i++)
        {
                ...
                if(readback == 0xfa) return 0; /* got the ack byte, bail out */
        }
...
ol_ps2_init_keyboard()
{
        ...
        if((!ol_ps2_init_dev(ol_ps2_keyboard, OL_PS2_KEYBOARD)) && (!ol_ps2_init_dev(
                ol_ps2_controller, OL_PS2_CONTROLLER)))
        {
                ol_ps2_keyboard->sent_command (ol_ps2_controller,
                                                OL_PS2_INIT_KB_CMD);                
        }
}
...
ol_detach_all_devices()
{
        ...
        if((ol_ps2_mouse->flags & OL_PS2_ACTIVE_FLAG) != 0 && 
                (ol_ps2_mouse != NULL))
        {
                free(ol_ps2_mouse);
        }
